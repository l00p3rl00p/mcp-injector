...
def _candidate_activator_bootstraps() -> list[Path]:
    ...
    # Workspace sibling (no upward search): <parent>/repo-mcp-packager/bootstrap.py
...
def main() -> int:
    ...
    if "--help" in sys.argv or "-h" in sys.argv:
        print("Nexus Bootstrap Forwarder (Injector)")
        print()
        ...
        print("  --repair                     Rebuild/update suite")
    ...
    if "--install-suite" in sys.argv:
        # non-interactive mode: install suite into ~/.mcp-tools without prompting
    ...
    print("  python3 mcp_injector.py --help")
    print("  python3 mcp_injector.py --list-clients")
    print()

    # Best-effort: install suite into central (no scanning) then re-run
