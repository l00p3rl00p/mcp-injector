...
* **Web-Client Proxy Best Practice**: For browser-based AI clients, the recommended pattern is to use a local MCP proxy that exposes SSE / Streamable HTTP / WebSocket endpoints, rather than injecting those transports into IDE MCP configs.
...
* **Success**: Running `uninstall.py` removes the `# Nexus Block` from `.zshrc` without deleting other aliases.
...
|---|---|---|
| Headless install (no prompts, no friction) | ✅ | 90% |
| Surgical IDE injection (zero JSON corruption) | ✅ | 88% |
| IDE detection (claude, cursor, antigravity) | ✅ | 90% |
